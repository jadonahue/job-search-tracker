...
export default function Button({ label, onClick, href, type = "button", className = "" }) {
    ...
    return (
        <button
            ...
            className={`bg-indigo-200 rounded-lg text-indigo-800 hover:bg-indigo-800 hover:text-indigo-200 hover:outline-indigo-200 hover:outline font-bold py-2 px-4 ${className}`}
            onClick={onClick}
        >
            {label}
        </button>
